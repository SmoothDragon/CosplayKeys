...
def heart2(R):
    ...
    shape = sd.intersection()(shape, sd.translate([0,-500])(sd.square(1000)))
...
if __name__ == '__main__':
    ...
    headpiece = sd.intersection()(headpiece, sd.translate([0,-500])(sd.square(1000)))
    ...
    cutout = sd.translate([0,0,4])(cutout)
    final = head - cutout
    final = sd.translate([0,1.3*R, 0])(final)
    final = sd.rotate([70, 0, 0])(final)
    final = sd.translate([0, -1.75*R, 0])(final)
    final = sd.union()(*[sd.rotate([0,0,i*72])(final) for i in range(5)])
    ss = .67
    final += sd.translate([0,0,2.0*R])(sd.rotate([0,0,36])(sd.scale(ss)(final)))
    final += sd.translate([0,0,(1+ss)*2.0*R])(sd.scale(ss*ss)(final)) + sd.translate([0,0,(1+ss+ss*ss+ss**3)*2.0*R])(sd.scale(ss*ss*ss*ss)(final))
    final = sd.translate([0,0,.8])(final)
    # final = sd.intersection()(final, sd.translate([0,0,500])(sd.cube(1000, center=True)))
    final = sd.scale(1.2)(final)
    final += sd.cylinder(h=3,r=2.5*R+.5,center=True)-sd.cylinder(h=6,r=2.5*R-5,center=True)
